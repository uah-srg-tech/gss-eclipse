...
public class GSSDescriptionAssociator {
	...
	public void linkDocumentation(EObject model) {
		...
		while(itor.hasNext()) {
			...
			if(grammarElement == slRule || grammarElement == mlRule) {
				...
				if(commentSemantic == semantic && ((semantic instanceof GSSModelElement) == false) &&
						x.getOffset() > NodeModelUtils.findActualNodeFor(semantic).getOffset())
					continue;
				
				if(semantic instanceof GSSModelObject) {
					GSSModelObject object = (GSSModelObject) semantic;
				...
				else if(semantic instanceof GSSModelElement) {
					GSSModelElement object = (GSSModelElement) semantic;
					// found sequence is documentation for semantic
					associateDescription(object, commentSequence);
					// need a new sequence, or the one just given away may be cleared
					commentSequence = Lists.newArrayList();
	...
	private void associateDescription(EObject object, List<INode> commentSequence) {
		...
		if (object instanceof GSSModelElement) {
			((GSSModelElement)object).setDescription(buf.toString());
		}
		else if (object instanceof GSSModelObject) {
			((GSSModelObject)object).setDescription(buf.toString());
		}
